(function () {
  ...
  try {
    ...
    if (!document.getElementById("pkcw-font-montserrat")) {
      document.head.appendChild(el("link", {
        id: "pkcw-font-montserrat",
        rel: "stylesheet",
    ...
    if (!document.getElementById("pkcw-style")) {
      var css = ""
        ...
        + ".pkcw-btn:focus-visible{box-shadow:" + CONFIG.theme.shadow + ",0 0 0 4px " + CONFIG.theme.ring + "}"
        ...
        + ".pkcw-close:focus{outline:none}"
        + ".pkcw-close:focus-visible{box-shadow:0 0 0 4px " + CONFIG.theme.ring + "}"
        ...
        + "@keyframes pkcw-trigger-icon-pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.12)}}"
        + "@media (prefers-reduced-motion: reduce){.pkcw-btn::after,.pkcw-btnIcon .pkcw-triggerPhone,.pkcw-labelMarquee{animation:none}}"
        + ".pkcw-root[data-open='1'] .pkcw-btn::after{opacity:0}"
        + "@media (max-width:480px){"
          + ".pkcw-root{--pkcw-btn:64px;right:28px;bottom:28px;width:var(--pkcw-btn)}"
          ...
          + ".pkcw-btnIcon{width:34px;height:34px}"
          + ".pkcw-btn.pkcw-btn--cross .pkcw-btnIcon{width:20px;height:20px}"
    ...
    openBtnIcon.innerHTML = iconSvg("phone");
    var openBtnTriggerIcon = openBtnIcon.querySelector("svg");
    if (openBtnTriggerIcon) openBtnTriggerIcon.classList.add("pkcw-triggerPhone");
    ...
    function makeAction(a) {
      ...
      if (a.marquee) {
        ...
        var lbl = a.label || "";
        var ratio = Math.max(1, lbl.length / 13);
    ...
    var i;
    for (i = 0; i < CONFIG.primaryActions.length; i++) {
      grid.appendChild(makeAction(CONFIG.primaryActions[i]));
    }
    for (i = 0; i < CONFIG.moreActions.length; i++) {
      grid.appendChild(makeAction(CONFIG.moreActions[i]));
    }
    ...
    on(
      ...
      "wheel",
      function (e) {
        if (root.getAttribute("data-open") === "1" && card.contains(e.target)) {
          e.preventDefault();
          e.stopPropagation();
        }
      },
